...
export const calcRenderColumns = (
  ...
  const isOnlyNeedTime = [
    AlDateTimePickerFineness.minute,
    AlDateTimePickerFineness.hour,
  ].includes(fineness as any);
  ...
  if (
    !isOnlyNeedTime &&
    ![AlDateTimePickerFineness.year, AlDateTimePickerFineness.month].includes(
      fineness as any,
    )
  ) {
  ...
  if (
    isOnlyNeedTime ||
    [
      AlDateTimePickerFineness.hourOfOneDay,
      AlDateTimePickerFineness.minuteOfOneDay,
    ].includes(fineness as any)
  ) {
...
export const calcValidColumnValues = (
  ...
  } else if (column === AlDateTimePickerColumn.month) {
  ...
  } else if (column === AlDateTimePickerColumn.date) {
    const belong = Dayjs()
