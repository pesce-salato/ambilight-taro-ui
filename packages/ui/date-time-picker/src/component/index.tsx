...
const defaultFormatter: AlDateTimePickerFormatter = (
  column,
  _columnIndex,
  { rowValue },
) => {
  const displayValue =
    column === AlDateTimePickerColumn.month ? rowValue + 1 : rowValue;
  return (
...
export const AlDatTimePicker = (originalProps: AlDateTimePickerProps) => {
  ...
  const { fineness, value, defaultValue, onChange, range, filter, formatter } =
    props;
  ...
  const toValid = useCallback(
    (needCheckValue: number[]) => {
      ...
      for (
        let columnIndex = 0;
        columnIndex < renderColumns.length;
        columnIndex++
      ) {
  ...
  }, [compatibleValue, renderColumns, formatter, getColumnRange]);

  return (
    <View className={root.className}>
      {pickers.map((picker, columnIndex) => (
        <View className={root.hierarchies("column").className}>
          <AlPicker
